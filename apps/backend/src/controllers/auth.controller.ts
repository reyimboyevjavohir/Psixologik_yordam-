...
export async function register(req: Request, res: Response) {
  try {
    ...
    res.status(201).json(ok(
      { accessToken, user: { id: user.id, fullName: user.fullName, username: user.username, email: user.email, role: user.role } },
      "Ro'yxatdan o'tildi"
    ));
  } catch (e) {
...
export async function login(req: Request, res: Response) {
  try {
    ...
    await prisma.refreshToken.create({
      data: { token: refreshTokenStr, userId: user.id, expiresAt: new Date(Date.now() + REFRESH_TTL_MS) },
    });
...
export async function me(req: Request, res: Response) {
  try {
    const user = await prisma.user.findUnique({
      ...
      select: { id: true, fullName: true, username: true, email: true, phone: true, avatarUrl: true, role: true, profile: true, createdAt: true, isActive: true },
    });
